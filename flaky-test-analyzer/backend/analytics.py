...
def initialize_storage() -> bool:
    """Create the intentionally minimal storage schema when analytics is enabled."""
        ...
        with _connect() as connection:
            connection.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_analysis_feedback_created_at
                    ON analysis_feedback(created_at);
                """
            )
        return True
    except (OSError, sqlite3.Error):
        logger.warning("Product analytics storage could not be initialized", exc_info=True)
        return False
...
def record_event(
    ...
    """Record only allow-listed product metadata; never accept diagnostic fields."""
    if not analytics_enabled():
        return False
    if event not in ALLOWED_EVENTS or (
        analysis_type is not None and analysis_type not in ALLOWED_ANALYSIS_TYPES
    ):
        ...
        with _connect() as connection:
            connection.execute(
                ...
                (
                    ...
                    event,
                    analysis_type,
                    None if ai_enabled is None else int(ai_enabled),
                    None if success is None else int(success),
                ),
            )
        return True
    except (OSError, sqlite3.Error):
        logger.warning("Product analytics event could not be recorded", exc_info=True)
        return False
        ...
        with _connect() as connection:
            connection.execute(
                """INSERT INTO analysis_feedback
                   ...
                   VALUES (?, ?, ?, ?, ?)""",
                (datetime.now(UTC).isoformat(), anonymous_session_id, int(useful), feedback_text, analysis_type),
            )
        return True
    except (OSError, sqlite3.Error):
        logger.warning("Product feedback could not be recorded", exc_info=True)
        return False
    ...
    try:
        ...
        with _connect() as connection:
            ...
            feedback = connection.execute(
                "DELETE FROM analysis_feedback WHERE created_at < ?", (cutoff,)
            ).rowcount
        return events + feedback
    except (OSError, sqlite3.Error):
        logger.warning("Expired product analytics could not be cleaned up", exc_info=True)
        return 0
    ...
    with _connect() as connection:
        counts = {
            ...
            for row in connection.execute(
                "SELECT event, COUNT(*) total FROM analytics_events WHERE created_at >= ? GROUP BY event",
                (cutoff,),
            )
        }
        type_counts = {name: 0 for name in sorted(ALLOWED_ANALYSIS_TYPES)}
        for row in connection.execute(
            """SELECT analysis_type, COUNT(*) total FROM analytics_events
               WHERE created_at >= ? AND event = ? GROUP BY analysis_type""",
            (cutoff, "analysis_completed"),
        ):
            if row["analysis_type"] in type_counts:
                type_counts[row["analysis_type"]] = row["total"]
        ...
        repeat_sessions = connection.execute(
            """SELECT COUNT(*) FROM (SELECT anonymous_session_id FROM analytics_events
               WHERE created_at >= ? AND event = ? GROUP BY anonymous_session_id HAVING COUNT(*) > 1)""",
            (cutoff, "analysis_completed"),
        ).fetchone()[0]
        page_sessions = connection.execute(
            """SELECT COUNT(DISTINCT anonymous_session_id) FROM analytics_events
               WHERE created_at >= ? AND event = ?""",
            (cutoff, "page_view"),
        ).fetchone()[0]
        sample_to_own = connection.execute(
            """SELECT COUNT(DISTINCT sample.anonymous_session_id)
               FROM analytics_events sample
               ...
               AND EXISTS (SELECT 1 FROM analytics_events own
                 ...
                 AND own.event = ? AND own.analysis_type != ?
                 AND own.created_at > sample.created_at)""",
            (cutoff, "sample_used", "analysis_completed", "sample"),
        ).fetchone()[0]
        feedback = connection.execute(
            """SELECT COALESCE(SUM(CASE WHEN useful = 1 THEN 1 ELSE 0 END), 0),
                      COALESCE(SUM(CASE WHEN useful = 0 THEN 1 ELSE 0 END), 0)
               FROM analysis_feedback WHERE created_at >= ?""",
            (cutoff,),
        ).fetchone()
    yes, no = feedback[0], feedback[1]
    total = yes + no
    return {
        "period_days": days,
        ...
        "failed_analyses": counts.get("analysis_failed", 0),
        "sample_uses": counts.get("sample_used", 0),
        "analysis_types": type_counts,
        "ai_requests": counts.get("ai_analysis_requested", 0),
        "ai_completed": counts.get("ai_analysis_completed", 0),
        "anonymous_sessions_with_analysis": completed_sessions,
